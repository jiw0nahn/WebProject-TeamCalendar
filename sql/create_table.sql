...
CREATE TABLE teams (
    `id` INT AUTO_INCREMENT PRIMARY KEY,
    `name` VARCHAR(50) NOT NULL,          -- 팀 이름 (예: 캡스톤디자인조)
...
/* 3. 팀 멤버 테이블 (teamMembers) */
/* 누가 어느 팀에 속해있는지 연결 (N:M) */
CREATE TABLE teamMembers (
    ...
    UNIQUE KEY (userId, teamId) -- 한 사람이 같은 그룹에 중복 가입 방지
);
...
CREATE TABLE todos (
    ...
    `teamId` INT DEFAULT NULL,           -- NULL이면 '개인 할 일', 값이 있으면 '그룹 할 일'
